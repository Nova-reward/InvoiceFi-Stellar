...
| Relative volume spike | `relative_volume_spike` comparing 5-minute volume against a configurable multiple of the 7-day average 5-minute volume | `ANOMALY_RELATIVE_VOLUME_SPIKE_MULTIPLE=3` | A volume spike relative to recent history may indicate a rapidly worsening market condition or an aggressive drawdown not caught by static thresholds. |
| Unusual discount rate | `unusual_discount` when a discount rate deviates by more than `ANOMALY_UNUSUAL_DISCOUNT_STD_DEV_MULTIPLE` standard deviations from the trailing 30-day mean | 3 standard deviations | Drastic changes in discount rates could signify broken pricing models or potentially an attempt to exploit risk assessment. |
